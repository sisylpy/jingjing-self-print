<template>
    ...
    <div class="applyBodyPage ">
        ...
        <ConfirmationModal :visible="isModalVisible" message="是否需要保存订单？" @confirm="handleConfirm"
            @cancel="handleCancel" />

        <!-- ...您的其他内容... -->

        <div id="printArea" class="print-area" style="display: flex;"></div>

        <!-- 打印纸尺寸信息 -->
        <!-- <div class="paper-size-info">
            打印纸尺寸: 241.3mm x 139.7mm<br>
...
export default {
    ...
    methods: {
        ...
        createHeader(pageIndex, totalPriPage) {
            const fontSize = this.disName.length > 6 ? '14px' : '24px';
            return `
            <div class="header" style="width: 100%; float:left;  ">
                <div style="width: 85%; margin-left: 7.5%; line-height: 36px; float:left; font-size: 14px; ">
                    <div style="width: 34%; float: left;">
                        <div style="line-height: 36px; font-size: 14px; border-left: 1px soild gray;">单位: ${this.depName}</div>
                    </div>
                    <div style="width: 33%; float: left; text-align: center; font-size: ${fontSize};">${this.disName}送货单</div>
        ...
        createFooter() {
    return `
    <div  style="width: 100%; float:left; font-size: 14px; border-top: 1px solid gray; ">
        <div style="box-sizing: border-box; float: left; width: 100%; line-height: 24px; font-size: 13px; text-align: right; padding-right: 3%;border-right: 1px solid gray;border-left: 1px solid gray;">
            ${this.numberToChinese(this.subtotal)} 合计: ${this.subtotal}
        </div>
        <div style="box-sizing: border-box; float: left; width: 100%; border-top: 1px solid gray;">
    ...
    `;
        ...
        creatPageFooter() {
            ...
            return `
            <div style="box-sizing: border-box; float: left; width: 100%;  line-height: 36px; font-size: 14px; border-top: 1px solid gray; left;">
                <div>地址: ${this.disUser.nxDistributerEntity.nxDistributerAddress}</div>
            </div>
            `;
        },
        ...
        createPageContent(pageIndex) {
            ...
            let pageContent = '<div style="float: left; width: 100%;  border-right: 1px solid gray;">'; // 使用 flex 容器
                
            const bottomTitle = (endIndex < 13) ? '' : 'border-bottom: 1px solid gray; padding-bottom: 2px';
            
             // 添加表头到每一列
            pageContent += `
        <div style="display: flex; text-align:center; font-size: 12px; width: 100%; box-sizing: border-box; border-top: 1px solid gray; border-top: 1px solid gray;">
            <div class="table-header" style="display: flex; text-align:center; width: 50%; ">
            ...
            <div class="table-header" style="display: flex; text-align:center; width: 50%;">
                <div style="box-sizing: border-box; float: left; width: 10%; border-right: 1px solid gray; ${bottomTitle} ">序号</div>
                <div style="box-sizing: border-box; float: left; width: 30%; border-right: 1px solid gray; ${bottomTitle} ">商品</div>
                <div style="box-sizing: border-box; float: left; width: 15%; border-right: 1px solid gray; ${bottomTitle} ">规格</div>
                <div style="box-sizing: border-box; float: left; width: 10%; border-right: 1px solid gray; ${bottomTitle} ">数量</div>
                <div style="box-sizing: border-box; float: left; width: 15%; border-right: 1px solid gray; ${bottomTitle} ">单价</div>
                <div style="box-sizing: border-box; float: left; width: 20%;  ${bottomTitle} ">小计</div>
            </div>
        </div>
    `;

            let orderIndex = startIndex; // 从当前页的开始位置初始化序号
            ...
            for (let col = 0; col < columnsPerPage; col++) {
                ...
                for (let row = 0; row < rowsPerPage; row++) {
                    ...
                    if (currentPageData[dataIndex]) {
                        ...
                        const subtotalBorderLeft = (col === 0) ? 'border-right: 1px solid gray;' : '';
                        const indexBorderLeft = (col === 0) ? 'border-left: 1px solid gray;' : '';
                        const rightBorderLeft = (col === 0) ? '' : 'border-right: 1px solid gray;';
                        columnContent += `
                    <div class="oneItem" style="float: left; width: 100%; border-top: 1px solid gray; line-height: 24px; text-align: center; font-size: 12px;">
                        <div style="word-break: keep-all; box-sizing: border-box; float: left; width: 10%;  ${rightBorderLeft} ${indexBorderLeft} ${subtotalBorderLeft} ">${orderIndex + 1}</div>
                        <div style="word-break: keep-all; box-sizing: border-box; float: left; width: 30%;  ${rightBorderLeft} ${subtotalBorderLeft} text-align: left; padding-left: 10px;">${rowData['nxDistributerGoodsEntity']['nxDgGoodsName']}</div>
                        <div style="word-break: keep-all; box-sizing: border-box; float: left; width: 15%;  ${rightBorderLeft} ${subtotalBorderLeft} ">${rowData['nxDoStandard']}</div>
                        <div style="word-break: keep-all; box-sizing: border-box; float: left; width: 10%;   ${rightBorderLeft} ${subtotalBorderLeft} ">${rowData['nxDoWeight']}</div>
                        <div style="word-break: keep-all; box-sizing: border-box; float: left; width: 15%;   ${rightBorderLeft} ${subtotalBorderLeft} ">${rowData['nxDoPrice']}</div>
                        <div style="word-break: keep-all; box-sizing: border-box; float: left; width: 20%;   ${subtotalBorderLeft} ">${rowData['nxDoSubtotal']}</div>
                    </div>
                `;

                        // 累加小计
                        pageSubtotal += Number(rowData['nxDoSubtotal']);
                        orderIndex++; // 序号增加
                    }
                }
            ...
            pageContent += '</div>'; // 关闭主容器

            return { html: pageContent, pageSubtotal: pageSubtotal };
        },
        ...
        generatePrintContent() {
            ...
            // 初始化 printPageSubtotal 数组
            const printPageSubtotal = [];
            ...
            for (let pageIndex = 1; pageIndex <= totalPages; pageIndex++) {
                ...
                // 调用 createPageContent 并获取 HTML 和 pageSubtotal
                const { html: pageContent, pageSubtotal } = this.createPageContent(pageIndex);

                // 将当前页的小计添加到 printPageSubtotal 数组
                printPageSubtotal.push(pageSubtotal);

                // 如果总页数大于1且当前页不是最后一页，添加 subtotal 部分
                const subtotalHtml = (isMultiplePages) ? `
            <div style="box-sizing: border-box; float: left; width: 100%; border-top: 1px solid gray; line-height: 24px; font-size: 13px; text-align: right;  border-left: 1px solid gray; border-right: 1px solid gray; padding-right: 2px; ">
                <div style="box-sizing: border-box; float: left; width: 100%; line-height: 24px; font-size: 13px; text-align: right; padding-right: 3%;">
                    ${this.numberToChinese(pageSubtotal)} 小计: ${pageSubtotal}元
                    </div>
            </div>
        ` : '';

                const footerHtml = (pageIndex === totalPages) ? this.createFooter() : this.creatPageFooter();

                const pageHtml = `
            <div id="page${pageIndex}" class="applyBody1">
                ${this.createHeader(pageIndex, totalPages)}
                <div class="content_bill" style="width: 90%; margin-left: 5%; float:">
                    ${pageContent}
                    ${subtotalHtml}
                   ${footerHtml}
                </div>
        ...
        printOnly() {
            ...
            window.electronAPI.sendPrintRequest(printContent,this.nxDepId);
            setTimeout(() => {
